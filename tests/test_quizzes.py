...
class QuizTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.monthly_book = MonthlyBookModel.objects.create(
            month=12,
            year=2024,
            title='Test Book',
        ...
        self.quiz = Quiz.objects.create(
            monthly_book=self.monthly_book,
            title='Test Quiz'
        )
    ...
    def test_list_quizzes(self):
        ...
        response = self.client.get('/api/quizzes/quizzes/')

        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_retrieve_quiz(self):
        """Test retrieving a specific quiz."""
        response = self.client.get(f'/api/quizzes/quizzes/{self.quiz.id}/')
    ...
    def test_submit_quiz_answers_incorrect(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        data = response.json()
        self.assertEqual(data['score'], 0)  # No correct answers
    ...
    def test_get_quiz_submission_by_quiz(self):
        ...
        response = self.client.get(
            f'/api/quizzes/submissions/by_quiz/?quiz_id={self.quiz.id}'
        )
    ...
    def test_get_quiz_submission_not_found(self):
        ...
        response = self.client.get(
            f'/api/quizzes/submissions/by_quiz/?quiz_id={fake_quiz_id}'
        )
